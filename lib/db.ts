...
export async function ensureSchema() {
  await db.executeMultiple(`
    ...
    INSERT OR IGNORE INTO user_profile (id, name, bio, total_earned)
    VALUES (1, 'Alex Hustle', 'High-end digital solutions for modern brands.', 0);
  `);
}
...
export async function createNotification(type: string, title: string, message: string, data: object = {}) {
  ...
  await db.execute({
    ...
    args: [nanoid(), type, title, message, JSON.stringify(data)],
  });
}
